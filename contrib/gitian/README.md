...
*Setup instructions for a Gitian build of Motif.*

Gitian is the deterministic build process that is used to build the Motif CLI
...
```bash
cp motif/contrib/gitian/gitian-build.py .
```
...
```bash
git clone git@github.com:motif-project/gitian.sigs.git
...
gpg --output $VERSION-linux/$NAME/motif-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/motif-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/motif-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/motif-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/motif-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/motif-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[motif-project/gitian.sigs](https://github.com/motif-project/gitian.sigs/) repository:
...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/motif-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/motif-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/motif-osx-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/motifmooo-motif/bitmotif -b motifmooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
