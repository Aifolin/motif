...
Core tests take longer than any other Motif tests, due to the high amount of computational work involved in validating core components.
...
To run only Motif's core tests (after building):
...
To run only Motif's crypto tests (after building):
...
```bash
motifd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `motif-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Motif's hash tests (after building):
...
To run only Motif's performance tests (after building):
...
To run only Motif's unit tests (after building):
